#!/usr/bin/env python3
"""Train a linear decoder on normalized full-field activity."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a linear decoder on normalized full-field activity."
    )
    parser.add_argument(
        "--data",
        default="recon/outputs/activity_lum_pairs_fullfield.pt",
    ...
    parser.add_argument(
        "--out-model",
        default="recon/outputs/linear_decoder_fullfield_norm.pt",
        help="Path to save the trained decoder state_dict.",
    )
    parser.add_argument(
        "--out-stats",
        default="recon/outputs/linear_decoder_fullfield_norm_stats.pt",
...
def main() -> None:
    ...
    print(
        f"X_flat stats: min={x_min:.6f} max={x_max:.6f} "
    ...
    xn_min = float(Xn.min())
    xn_max = float(Xn.max())
    xn_mean = float(Xn.mean())
    xn_std = float(Xn.std(unbiased=False))
    print(
        f"Xn stats: min={xn_min:.6f} max={xn_max:.6f} "
        f"mean={xn_mean:.6f} std={xn_std:.6f}"
    )
