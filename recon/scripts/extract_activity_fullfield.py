#!/usr/bin/env python3
"""Extract aligned (activity, lum) pairs using full-field early visual activity.

This script is intentionally separate from the central-only extraction to allow
direct comparison of spatially rich vs. central-column activity.
Its purpose is to test whether spatial coverage enables luminance reconstruction.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract full-field (activity, lum) pairs from a pretrained FlyVis network."
    )
    parser.add_argument(
        "--network",
    ...
    parser.add_argument(
        "--output-name",
        default="activity_lum_pairs_fullfield.pt",
        help="Output filename for torch.save.",
    )
    parser.add_argument(
        "--indices",
        default=None,
        help="Path to JSON file containing dataset indices to extract.",
    )
    parser.add_argument(
        "--indices-key",
        default=None,
        help="Key to select indices from a JSON dict (e.g., train_indices or test_indices).",
    )
...
def main() -> None:
    ...
    if args.indices:
        ...
        with indices_path.open("r", encoding="utf-8") as handle:
            indices_obj = json.load(handle)
        if isinstance(indices_obj, list):
            indices = indices_obj
        elif isinstance(indices_obj, dict):
            if args.indices_key:
                if args.indices_key not in indices_obj:
                    raise KeyError(
                        f"Key {args.indices_key} not found in {indices_path}"
                    )
                indices = indices_obj[args.indices_key]
            elif "indices" in indices_obj:
                indices = indices_obj["indices"]
            elif len(indices_obj) == 1:
                indices = next(iter(indices_obj.values()))
            else:
                raise ValueError(
                    f"Indices file contains multiple keys; set --indices-key "
                    f"(found {list(indices_obj.keys())})"
                )
        else:
            raise TypeError(f"Unsupported indices format: {type(indices_obj)}")
    ...
    for count, idx in enumerate(indices):
        ...
        for cell_type in cell_types:
            act = getattr(activity, cell_type)
            feats.append(_as_feature_dim(act))

        X = torch.cat(feats, dim=-1).cpu()
        Y = lum.unsqueeze(0).cpu()

        print(f"sample {count} (dataset idx {idx}): X {tuple(X.shape)} | Y {tuple(Y.shape)}")
