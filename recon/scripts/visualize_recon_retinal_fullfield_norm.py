#!/usr/bin/env python3
"""Visualize normalized full-field reconstructions on FlyVis retinal representation."""
...
MODEL_PATH = Path("recon/outputs/linear_decoder_fullfield_norm.pt")
STATS_PATH = Path("recon/outputs/linear_decoder_fullfield_norm_stats.pt")
SAMPLE_INDEX = 0
...
def main() -> None:
    ...
    x_sample_norm = (x_sample - mean) / (std + eps)

    with torch.no_grad():
        y_hat = model(x_sample_norm).cpu()
